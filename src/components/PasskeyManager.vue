<template>
  <div class="bg-white rounded-lg shadow-md p-6 space-y-4">
    <h2 class="text-xl font-semibold text-gray-900">🔑 Mes appareils</h2>
    ...
    <template v-else>
      <p class="text-sm text-gray-600">
        Enregistre cet appareil pour te connecter sans code, avec ton empreinte ou ton visage.
      </p>

      <div v-if="message" class="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded" role="status">
        {{ message }}
      </div>
      ...
      <ul v-if="passkeys.length" class="divide-y divide-gray-200">
      ...
      <button
        ...
        class="w-full bg-indigo-600 text-white py-3 px-4 rounded-lg font-semibold hover:bg-indigo-700 disabled:bg-gray-300 disabled:cursor-not-allowed transition-colors"
      >
